      ...
      <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
        ...
        <div className="hidden lg:flex items-center gap-8 text-sm font-medium text-white/70">
          ...
          <div className="flex items-center gap-6 border-l border-white/10 pl-8 ml-4">
            <Link to="/cart" aria-label={`Carrito${getItemCount() > 0 ? `, ${getItemCount()} artículos` : ''}`} className="relative group p-2">
              <ShoppingCart className="w-5 h-5 text-white/70 group-hover:text-white transition-colors" aria-hidden="true" />
              {getItemCount() > 0 && (
                <span aria-hidden="true" className="absolute top-0 right-0 bg-brand-purple text-white text-[10px] font-bold min-w-[18px] h-[18px] rounded-full flex items-center justify-center animate-pulse border-2 border-[#050505]">
                  {getItemCount()}
                </span>
              )}
            </Link>

            <div role="group" aria-label="Selector de idioma" className="flex items-center gap-2 bg-white/5 rounded-full px-3 py-1.5 border border-white/5">
              <button
                onClick={() => setLang('es')}
              ...
              <span aria-hidden="true" className="w-px h-2 bg-white/10" />
              <button
                onClick={() => setLang('en')}
        ...
        <div className="flex items-center gap-4 lg:hidden relative z-50">
          <Link to="/cart" aria-label={`Carrito${getItemCount() > 0 ? `, ${getItemCount()} artículos` : ''}`} className="relative p-2">
            <ShoppingCart className="w-6 h-6 text-white/70" aria-hidden="true" />
            {getItemCount() > 0 && (
              <span aria-hidden="true" className="absolute top-1 right-1 bg-brand-purple text-white text-[10px] font-bold min-w-[18px] h-[18px] rounded-full flex items-center justify-center border-2 border-[#050505]">
                {getItemCount()}
              </span>
            )}
          </Link>
